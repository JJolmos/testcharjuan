import { Component, ViewChild, ElementRef, OnInit } from '@angular/core';
import { Chart, ChartDataSets, ChartData, ChartOptions } from 'chart.js';

@Component({
  selector: 'app-root',
  templateUrl: './app.component.html',
  styleUrls: ['./app.component.scss']
})
export class AppComponent implements OnInit {

  @ViewChild('grafica', { static: true }) graficaREF: ElementRef;

  ngOnInit() {

    // CHARTDATASET CONFIGURATION
    const datasets: ChartDataSets[] = [
      {
        label: 'Fan page',
        fill: false,
        data: [292.67, 292.82, 292.54, 292.54, 292.95, 292.41, 292.68, 292.27, 292.41, 292.21, 292.37, 292.50],
        backgroundColor: '#4C4CD8',
        borderColor: '#4C4CD8',
        borderWidth: 1,
        yAxisID: 'Fan page',
        //JJOlmos 02/08/2021 Change point Style
        pointBackgroundColor: "#4C4CD8",
        pointRadius: 3,
        pointHoverBackgroundColor: "#4C4CD8",
        pointHoverBorderColor: "rgba(60,60,216,0.6)",
        pointHoverBorderWidth: 9,
        pointHoverRadius: 9,
      },
      {
        label: 'Engaged users',
        fill: false,
        data: [21.43, 21.43, 26.78, 16.07, 16.07, 10.71, 10.71, 21.43, 5.36, 10.71, 5.36, 10],
        backgroundColor: '#F8CB1C',
        borderColor: '#F8CB1C',
        borderWidth: 1,
        yAxisID: 'Engaged users',
        //JJOlmos 02/08/2021 Change point Style
        pointBackgroundColor: "#F8CB1C",
        pointRadius: 3,
        pointHoverBackgroundColor: "#F8CB1C",
        pointHoverBorderColor: "rgba(200,150,28,0.6)",
        pointHoverBorderWidth: 9,
        pointHoverRadius: 9,
      }
    ]

    // CHARTDATA
    const data: ChartData = {
      labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dic'],
      datasets
    }

    // CHARTOPTIONS
    const options: ChartOptions = {
      responsive: false,
      legend: {
        display: false
      },
      hover: {
        mode: 'x',
        intersect: false,
        //JJOlmos 02/08/2021 Change Label Color 
        onHover: (evt, item) => {
          if (item.length) {
            console.log("onHover", item, evt.type);
            console.log(">data", item[0]._index, data.datasets[0].data[item[0]._index]);
          }
        }
      },
      scales: {
        xAxes: [
          {
            ticks: {
              fontColor: '#EBEBEB',
              callback: (value) => {
                return value
              },
              fontSize: 10,
              fontFamily: 'Work Sans',
              padding: 6
            },
            gridLines: {
              display: false,
            },
          },

        ],
        yAxes: [{
          id: 'Fan page',
          position: 'left',
          //JJOlmos 02/08/2021 set scale to 7 
          afterBuildTicks: (scale, values) =>
            this.setScale(values, 7),
          ticks: {
            fontColor: '#EBEBEB',
            padding: 15,
            fontSize: 10,
            fontFamily: 'Work Sans',
            callback: (value: string, index, values) => {
              const newValue = parseFloat(value).toFixed(2)
              return newValue + ' K'
            }
          },
          gridLines: {
            borderDash: [1, 2],
            color: "#282828",
            drawBorder: false,
            tickMarkLength: 0,
          }
        },
        {
          id: 'Engaged users',
          position: 'right',
          //JJOlmos 02/08/2021 set scale to 7 
          afterBuildTicks: (scale, values) =>
            this.setScale(values, 7),
          ticks: {
            fontColor: '#B8B7B7',
            fontSize: 10,
            padding: 15,
            fontFamily: 'Work Sans',
            callback: (value: string, index, values) => {
              const newValue = parseFloat(value).toFixed(2)
              return newValue
            },
          },
          gridLines: {
            drawBorder: false,
            tickMarkLength: 0,
            color: 'transparent'
          }
        }]
      },
      tooltips: {
        enabled: false,
        mode: 'x',
        intersect: false
      }
    }

    // INIT GRAPH
    new Chart('grafica', {
      type: 'line',
      data,
      options
    })

  }
  /**
   * 
   * @param {Array} values an array of scalar values 
   * @param {Number} scale the number of desired scale
   * @returns {Array} collection of ticks
   * @description Sets the scale on axis Y 
   * @author JJOlmos 
   */
  setScale(values, scale) {
    return new Array(scale).fill(values[0]).map(
      function (el, i) {
        return el + i * ((values[values.length - 1] - values[0]) / scale)
      });
  }
}
